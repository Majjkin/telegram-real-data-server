#!/bin/bash
# Полная настройка VPS для Creative MVP

echo "🚀 Setting up Creative MVP on VPS..."
...
mkdir -p /opt/creative-mvp
cd /opt/creative-mvp

# Создаем vps_server.py
...
# Создаем скрипт создания сессии
echo "📝 Creating session creation script..."
...
session_name = "telegram_session_vps"
...
cat > /etc/systemd/system/creative-mvp.service << 'EOF'
[Unit]
Description=Creative MVP Server
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/opt/creative-mvp
...
chmod +x vps_server.py
chmod +x create_session.py

echo "✅ Setup complete!"
echo ""
echo "🔧 Next steps:"
echo "1. Run: python3 create_session.py"
echo "2. Update .env with the session string"
echo "3. Run: python3 vps_server.py"
echo "4. Open: http://31.58.87.15:8000/ui"
echo ""
echo "🎯 For auto-start:"
echo "systemctl daemon-reload"
echo "systemctl enable creative-mvp"
echo "systemctl start creative-mvp"
